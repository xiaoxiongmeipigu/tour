package com.zjhj.tour.widget.picker;
...
public class WheelView extends View {
	...
	public static int VALUE_TEXT_COLOR = 0xe0000000;//选中的字体颜色
	private static final int ITEMS_TEXT_COLOR = 0xe0a6a9af;// 0xe0000000
	...
	//
	private OnScrollingStop onScrollingStop = null;
...
//				width += LABEL_OFFSET;
		...
		if (recalculate) {
			// recalculate width
			int pureWidth = width - 3*LABEL_OFFSET - 2 * PADDING;//- LABEL_OFFSET
			if (pureWidth <= 0) {
...
//widthLabel > 0 ? Layout.Alignment.ALIGN_OPPOSITE
		...
		if (!isScrollingPerformed
			...
			String text = getAdapter() != null ? getAdapter().getItem(
					...
					valueLayout = new StaticLayout(text != null ? text : "",
							...
							Layout.Alignment.ALIGN_CENTER, 1,height , false);//height  1
		...
		if (widthLabel > 0) {
			if (labelLayout == null || labelLayout.getWidth() > widthLabel) {
				...
				labelPaint.setColor(Color.parseColor("#1092fe"));//(VALUE_TEXT_COLOR);
				labelPaint.setTextSize(12 * dm.density);
				labelPaint.drawableState = getDrawableState();
				
				labelLayout = new StaticLayout(label, labelPaint, widthLabel,
						Layout.Alignment.ALIGN_CENTER, 1, height, false);//Layout.Alignment.ALIGN_NORMAL    valuePaint
			} else {
	...
	private void drawValue(Canvas canvas) {
		DisplayMetrics dm = this.getResources().getDisplayMetrics();
		valuePaint.setColor(Color.parseColor("#1092fe"));//(VALUE_TEXT_COLOR);
		valuePaint.setTextSize(13 * dm.density);
	...
	Handler handler = new Handler(){
		public void handleMessage(Message msg) {
			if (msg.what == 1) {
				onScrollingStop.onScrollingStop();
			}
		};
	};
	...
	public void setOnScrollingStop(OnScrollingStop onScrollingStop){
		this.onScrollingStop = onScrollingStop;
	}
